...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    for x, item in enumerate(items[:len(items)/2]):
        temp = items[x]
        items[x] = items[-1-x]
        items[-1-x] = temp
...
def duplicates(items):
    ...
    count_duplicates = {}
    for item in items:
        if item in count_duplicates:
            count_duplicates[item] += 1
        else:
            count_duplicates[item] = 1

    my_duplicates = []

    for item in count_duplicates:
        if count_duplicates[item] > 1:
